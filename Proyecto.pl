...
turno(Agente,T_p,T,D):-
    % random_agent(Accion,L,T_p,T),
    CallAgente=..[Agente,Accion,L,T_p,T],
...
simulacion(Agente,N,M,PS,PO,PN,T,C,T,_):-
    ...
    simulacion(Agente,N,M,PS,PO,PN,T,C_1,0,1).
simulacion(Agente,N,M,PS,PO,PN,T,C,T_p,1):-
    p_casillas_sucias(P),
    P<0.6,!,
    turno(Agente,T_p,T,D),
    % graficar(),
    T_p_1 is T_p+1,
    simulacion(Agente,N,M,PS,PO,PN,T,C,T_p_1,D).
simulacion(_,_,_,_,_,_,_,_,_,1):-
